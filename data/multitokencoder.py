...
class MaskedMidiEncoder(MultiTokEncoder):
    def __init__(self, quantization_cfg: DictConfig, time_quantization_method: str, masking_probability: float = 0.1):
        super().__init__()
        self.quantization_cfg = quantization_cfg
        self.time_quantization_method = time_quantization_method
        self.time_key = self.time_quantization_method + "_bin"
    ...
    def tokenize_src(self, record: dict) -> list[str]:
        tokens = self.tokenize_tgt(record)
        num_masks = self.masking_probability * len(tokens)

        ids_to_mask = np.random.randint(len(tokens), size=int(num_masks))
        for idx in ids_to_mask:
            tokens[idx] = "<MASK>"
    ...
    def untokenize_tgt(self, tokens: list[str]) -> pd.DataFrame:
        return self.untokenize_src(tokens)
